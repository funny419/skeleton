...
public class ClassUtil {
    ...
    public static Class<?> getArrayClass(Class<?> componentClass,int dimension) {
        ...
        switch (dimension) {
            ...
            default:
                Assert.assertTrue("wrong dimension:",dimension > 0);
    ...
    protected static class TypeInfo {
        ...
        private TypeInfo(Class<?> type) {
            ...
            if (dimension == 0) {
                ...
                for (Class<?> typeInterface : superclasses) {
                    set.addAll(getTypeInfo(typeInterface).interfaces);
                }
                ...
                for (Class<?> componentInterface : getTypeInfo(componentType).interfaces) {
    ...
    static {
        ...
        Assert.assertEquals(8,assignmentTable.size());
    }
    ...
    public static String getSimpleMethodSignature(Method method,boolean withModifiers,boolean withReturnType,boolean withClassName,boolean withExceptionType) {
        ...
        for (int i=0,cnt=paramTypes.length;i<cnt;i++) {
        ...
        if (withExceptionType) {
            ...
            if (ArrayUtils.isNotEmpty(exceptionTypes)) {
                ...
                for (int i=0,cnt=exceptionTypes.length;i<cnt;i++) {
